...
.PHONY: controller clean data_files warmboot_demos
...
.ONESHELL:
controller: build/full_demo.bin
	@if [ "$(DEVICE)" = "hx1k" ]; then \
		iceprog build/full_demo.bin; \
...
# yosys interprets the last verilog file listed as the top module$(USE_SPRAM)
	if [ "$(BRAM)" = "implicit" ]; then \
		...
		yosys -q -D NUM_BLOCKS_PARAM=$(NUM_BLOCKS) -D CLOCK_SPEED=$(CLOCK_SPEED) $(USE_SPRAM) \
		-D BRAM_INCLUDE_FILE="\"build/generated_rams.vh\"" -p 'synth_ice40 -json build/controller.json' \
...
# --pcf : pin constraints file for i/o
	nextpnr-ice40 -q $(PNR_PARAMS) --asc build/controller.asc --json build/controller.json --pcf $(PCF_FILE) --report report.json
...
build/generated_rams.vh:
	python3 src_python/generate_explicit_bram.py --n $(NUM_BLOCKS) --d $(DEVICE) --o build/generated_rams.vh
