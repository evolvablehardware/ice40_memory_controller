...
build/controller.bin: build $(HELPER_VERILOG) $(BRAM_INSTS) src_verilog/integrated_memory_controller.v build/data.hex 
# synthesis
...
# yosys interprets the last verilog file listed as the top module
	if [ "$(BRAM)" = "implicit" ]; then \
		yosys -q -D NUM_BLOCKS_PARAM=$(NUM_BLOCKS) -D CLOCK_SPEED=$(CLOCK_SPEED) -p 'synth_ice40 -json build/controller.json' \
		...
		yosys -q -D NUM_BLOCKS_PARAM=$(NUM_BLOCKS) -D CLOCK_SPEED=$(CLOCK_SPEED) \
...
# --pcf : pin constraints file for i/o
	nextpnr-ice40 $(PNR_PARAMS) --asc build/controller.asc --json build/controller.json --pcf $(PCF_FILE)
